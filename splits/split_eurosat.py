...
def update_classname(dataset_old):
    ...
    for item_old in dataset_old:
        cname_old = item_old[2]
        cname_new = NEW_CLASSNAMES[cname_old]
        item_new = (item_old[0], item_old[1], cname_new)
        dataset_new.append(item_new)
    return dataset_new

root = "/DFGZL/datasets/"
dataset_dir = os.path.join(root, "eurosat")
image_dir = os.path.join(dataset_dir, "2750")
split_path = os.path.join(dataset_dir, "split_EuroSAT.json")
