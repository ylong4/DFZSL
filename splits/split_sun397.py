...
root = "/DFGZL/datasets/"
dataset_dir = os.path.join(root, "sun397")
image_dir = os.path.join(dataset_dir, "SUN397")
split_path = os.path.join(dataset_dir, "split_SUN397.json")
...
if os.path.exists(split_path):
    ...
    classnames = []
    with open(os.path.join(dataset_dir, "ClassName.txt"), "r") as f:
        lines = f.readlines()
        for line in lines:
            line = line.strip()[1:]  # remove /
            classnames.append(line)
    cname2lab = {c: i for i, c in enumerate(classnames)}
    trainval = read_data(dataset_dir, image_dir, cname2lab, "Training_01.txt")
    test = read_data(dataset_dir, image_dir, cname2lab, "Testing_01.txt")
