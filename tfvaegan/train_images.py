...
opt.image_embedding="vlpt-simulator-ViTB16"
opt.class_embedding="vlpt-simulator-clip"

print(opt.dataset,opt.nclass_all)


logger = logging.getLogger()
logger.setLevel(logging.INFO)
logfile = "/DFGZL/log/tfvaegan/"+ opt.dataset + ".txt"
fh = logging.FileHandler(logfile, mode='a')
fh.setLevel(logging.INFO)
formatter = logging.Formatter("%(asctime)s - [line:%(lineno)d]: %(message)s")
logger.addHandler(fh)
logger.info("tfvaegan")
...
def loss_fn(recon_x, x, mean, log_var):
    ...
    MSE= torch.nn.functional.mse_loss(recon_x, x.detach(), size_average=False)
...
def save_mat(train_X, train_Y):
    mat_filepath = os.path.join(opt.dataroot, opt.dataset, opt.image_embedding + '.mat')
    mat_backbone = sio.loadmat(mat_filepath)
    feature = mat_backbone['features'].T
    label = mat_backbone['labels'].astype(int).squeeze()

    mat_filepath = os.path.join(opt.dataroot, opt.dataset, opt.class_embedding + '_splits.mat')
    ...
    x, y = train_X.numpy(), train_Y.numpy() + 1
    save_name = 'tfvaegan-' + opt.image_embedding + '.mat'
    save_features = np.vstack((x, test_seen_feature, test_unseen_feature))
    save_labels = np.hstack((y, test_seen_label, test_unseen_label))
    mat_backbone['features'] = save_features.T
    mat_backbone['labels'] = save_labels
    save_file = os.path.join(opt.dataroot, opt.dataset, save_name)
    sio.savemat(save_file, mat_backbone)

    save_name = 'tfvaegan-' + opt.class_embedding + '_splits.mat'
...
best_acc_seen, best_acc_unseen = 0, 0
best_gzsl_acc_per_class = 0
best_zsl_acc_per_class = 0

best_acc_base, best_acc_novel = 0, 0
best_gzsl_acc = 0
best_zsl_acc = 0
print(f"---------------opt.image_embedding:{opt.image_embedding}-------------------------------")
for epoch in range(0,opt.nepoch):
    ...
    print('[%d/%d]  Loss_D: %.4f Loss_G: %.4f, Wasserstein_dist:%.4f, vae_loss_seen:%.4f' % (epoch, opt.nepoch, D_cost.item(), G_cost.item(), Wasserstein_D.item(), vae_loss_seen.item()))
    netG.eval()
    netDec.eval()
    netF.eval()
    ...
    if opt.gzsl:   
        ...
        gzsl_cls = classifier.CLASSIFIER(train_X, train_Y, data, nclass, opt.cuda, opt.classifier_lr, 0.5, \
                100, opt.syn_num, generalized=True, netDec=netDec, dec_size=opt.attSize, dec_hidden_size=4096, ratio=opt.ratio)

        if best_gzsl_acc_per_class < gzsl_cls.H:
            best_acc_seen, best_acc_unseen, best_gzsl_acc_per_class = gzsl_cls.acc_seen, gzsl_cls.acc_unseen, gzsl_cls.H
            # save_mat(train_X, train_Y)
        print(opt.image_embedding)
        print('Here: seen=%.4f, unseen=%.4f, h=%.4f' % (gzsl_cls.acc_seen, gzsl_cls.acc_unseen, gzsl_cls.H))
        print('Best: seen=%.4f, unseen=%.4f, h=%.4f' % (best_acc_seen, best_acc_unseen, best_gzsl_acc_per_class))

        if best_gzsl_acc < gzsl_cls.HM:
            best_acc_base, best_acc_novel, best_gzsl_acc = gzsl_cls.acc_base, gzsl_cls.acc_novel, gzsl_cls.HM

        print('Here: base=%.4f, novel=%.4f, hm=%.4f' % (gzsl_cls.acc_base, gzsl_cls.acc_novel, gzsl_cls.HM))
        print('Best: base=%.4f, novel=%.4f, hm=%.4f' % (best_acc_base, best_acc_novel, best_gzsl_acc))

        if (epoch + 1) % 5 == 0:
            logger.info('Here: seen=%.4f, unseen=%.4f, h=%.4f' % (gzsl_cls.acc_seen, gzsl_cls.acc_unseen, gzsl_cls.H))
            logger.info('Best: seen=%.4f, unseen=%.4f, h=%.4f' % (best_acc_seen, best_acc_unseen, best_gzsl_acc_per_class))
            logger.info("--"*16)
            logger.info('Here: base=%.4f, novel=%.4f, hm=%.4f' % (gzsl_cls.acc_base, gzsl_cls.acc_novel, gzsl_cls.HM))
            logger.info('Best: base=%.4f, novel=%.4f, hm=%.4f' % (best_acc_base, best_acc_novel, best_gzsl_acc))
